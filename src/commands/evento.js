...
module.exports = {
    ...
    async handleInteraction(interaction) {
        ...
        if (interaction.isButton() && interaction.customId === 'abrir_modal_evento') {
            ...
            modal.addComponents(
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('modos').setLabel('Modos').setStyle(TextInputStyle.Short).setRequired(true)),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('horario').setLabel('Horário').setStyle(TextInputStyle.Short).setRequired(true)),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('premio_input').setLabel('Prêmio').setStyle(TextInputStyle.Short).setRequired(true)),
        ...
        // 5. FINALIZAÇÃO + LOG (REVISADO)
        if (interaction.type === InteractionType.ModalSubmit && interaction.customId.startsWith('modal_venc_')) {
            const data = interaction.customId.split('_');
            const criadorId = data[2];
            const premio = data[3];
            ...
            const embedLog = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '🎁 Prêmio:', value: `> ${premio || 'Não definido'}`, inline: false },
                    { name: '📅 Data:', value: `\`${dataFormatada}\``, inline: false }
                )
                .setTimestamp();

            // Busca forçada do canal de log
            try {
                ...
                console.error("Erro ao enviar log para a staff:", err);
            }
